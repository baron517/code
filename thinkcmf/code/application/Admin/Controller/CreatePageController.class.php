...
class CreatePageController extends AdminbaseController {
	...
	public function index(){
       ...
       for($i=0;$i<count($list);$i++)
       {
           ...
           {
               $zhushi=$list[$i]["comment"];
               $zhushiList=json_decode($zhushi,1);
               if(!$zhushiList["no_create"])
               {
	...
	public function add_post(){
		...
		if (IS_POST) {
            ...
            foreach ($_POST['post'] as $i => $v)
            {
                
                 $sql="select COLUMN_NAME as ziduan,column_comment as zhushi from INFORMATION_SCHEMA.Columns where table_schema='".$dbName."' and table_name='".$_POST['post'][$i]."'";
    		    $list=$db->query($sql);
                ...
                for($k=0;$k<count($list);$k++)
                {
                    ...
                    if($k>0)
                    {
                        ...
                        if(!$zhushiList["table_hide"])
                        {
                            ...
                            else if($zhushiList["upload"]=="video")
                            {
                                
                                $tableBody=$tableBody.
                                '
                                <td><a target="_blank" href="{$vo.'.$ziduan.'}">预览</a></td>';
                            ...
                            else if($zhushiList["type"])
                            {
                                
                                $tableBody=$tableBody.
                                '
                                <td class="status-text"><span class="hide" data-value="{$vo.'.$ziduan.'}">'.json_encode($zhushiList["type"],JSON_UNESCAPED_UNICODE).'</span></td>';
                            ...
                            if($zhushiList["date"])
                           {
                            ...
                            '
                             <span>
                			<label>'.$label.'</label>
                			<input type="text" class="laydate-range"  name="post['.$ziduan.'-range]" style="width: 200px;" value="{$post.'.$ziduan.'-range|default=""}">
                			</span>';
                               ...
                               else if($zhushiList["upload"]=="video")
                               {
                                    $createform=$createform.'
                                     <tr>
            							<th>'.$label.'</th>
            							<td>
            							    <span class="upload-col" >选择视频
            								<input class="file" type="file"  data-type="video" style="width:400px;" id="'.$ziduan.'"  /><input type="hidden" name="post['.$ziduan.']"   value="{$post.'.$ziduan.'}"></span><a target="_blank" class="file-video" href="{$post.'.$ziduan.'}"></a> '.$zhushiList["info"].'
            							</td>
            						</tr>';
                               }
                           ...
                           else if($zhushiList["textarea"])
                           {
                            ...
                            $createform=$createform.'
                             <tr>
    							<th>'.$label.'</th>
    							<td>
    							<script type="text/plain" id="'.$ziduan.'" name="post['.$ziduan.']">{$post.'.$ziduan.'}</script>
    						...
    						$editorScript=$editorScript.'
                				...
                				editorcontent_'.$ziduan.' = new baidu.editor.ui.Editor();
                				editorcontent_'.$ziduan.'.render("'.$ziduan.'");
                				try {
                					editorcontent_'.$ziduan.'.sync();
                				} catch (err) {
                				}';
    						
    						
    						$phpEditor=$phpEditor."
    						\$article['".$ziduan."']=htmlspecialchars_decode(\$article['".$ziduan."']);";
                             ...
                             <tr>
    							...
    							<td>
    								<input type="text" style="width:400px;" id="'.$ziduan.'" name="post['.$ziduan.']"   value="{$post.'.$ziduan.'}" />'.$zhushiList["info"].'
    							</td>
    						</tr>';
                ...
                if($zhushi1["daochu_table"])
                {
                    $daochuHtml="<a class='btn btn-danger export-excel' data-table={$zhushi1['daochu_table']}>导出excel</a>";
                }
...
<body data-url="{$serverUrl}">
	...
	<script type="text/javascript" src="__PUBLIC__/js/main.js?v=1"></script>
