...
class TagController extends AdminbaseController {
	...
	public function index(){
		$this->_lists($_POST["keyword"],$_POST["type"]);
		$this->display();
	}
	...
	public function add_post(){
		...
		if (IS_POST) {
			...
			$result=$this->Tag_model->add($article);
	...
	public function edit(){
		...
		$post=$this->Tag_model->where("t_id=$id")->find();
	...
	public function edit_post(){
		if (IS_POST) {
			...
			$article['detail']=htmlspecialchars_decode($article['detail']);
			$result=$this->Tag_model->save($article);
			if ($result!==false) {
				$this->success("保存成功！");
			} else {
				$this->error("保存失败！");
	...
	private function _lists($keyword,$type){
		...
		$posts=$this->Tag_model->select();

		//echo $this->Tag_model->getLastSql();
	...
	public function delete(){
		if(isset($_GET['id'])){
			$id = I("get.id",0,'intval');
			if ($this->Tag_model->where(array('t_id'=>$id))->delete() !==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
		...
		if(isset($_POST['ids'])){
			...
			if ($this->Tag_model->where(array('id'=>array('in',$ids)))->delete()!==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
	...
	public function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			if ( $this->Tag_model->where(array('id'=>array('in',$ids)))->save(array('post_status'=>1)) !== false ) {
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["uncheck"]){
			...
			if ( $this->Tag_model->where(array('id'=>array('in',$ids)))->save(array('post_status'=>0)) !== false) {
				$this->success("取消审核成功！");
			} else {
				$this->error("取消审核失败！");
			}
	...
	public function top(){
		if(isset($_POST['ids']) && $_GET["top"]){
			...
			if ( $this->Tag_model->where(array('id'=>array('in',$ids)))->save(array('istop'=>1))!==false) {
				$this->success("置顶成功！");
			} else {
				$this->error("置顶失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["untop"]){
			...
			if ( $this->Tag_model->where(array('id'=>array('in',$ids)))->save(array('istop'=>0))!==false) {
				$this->success("取消置顶成功！");
			} else {
				$this->error("取消置顶失败！");
			}
	...
	public function recommend(){
		if(isset($_POST['ids']) && $_GET["recommend"]){
			...
			if ( $this->Tag_model->where(array('id'=>array('in',$ids)))->save(array('recommended'=>1))!==false) {
				$this->success("推荐成功！");
			} else {
				$this->error("推荐失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["unrecommend"]){
			...
			if ( $this->Tag_model->where(array('id'=>array('in',$ids)))->save(array('recommended'=>0))!==false) {
				$this->success("取消推荐成功！");
			} else {
				$this->error("取消推荐失败！");
			}
	...
	public function copy(){
		if(IS_POST){
			if(isset($_GET['ids']) && isset($_POST['term_id'])){
				...
				foreach ($ids as $id){
					$find_post=$this->Tag_model->field('post_keywords,post_source,post_content,post_title,post_excerpt,smeta')->where(array('id'=>$id))->find();
					if($find_post){
						$find_post['post_author']=$uid;
						$find_post['post_date']=date('Y-m-d H:i:s');
						$find_post['post_modified']=date('Y-m-d H:i:s');
						$post_id=$this->Tag_model->add($find_post);
						if($post_id>0){
	...
	public function clean(){
		if(isset($_POST['ids'])){
			$ids = I('post.ids/a');
			$ids = array_map('intval', $ids);
			$status=$this->Tag_model->where(array("id"=>array('in',$ids),'post_status'=>3))->delete();
			...
			if(isset($_GET['id'])){
				$id = I("get.id",0,'intval');
				$status=$this->Tag_model->where(array("id"=>$id,'post_status'=>3))->delete();
	...
	public function restore(){
		if(isset($_GET['id'])){
			$id = I("get.id",0,'intval');
			if ($this->Tag_model->where(array("id"=>$id,'post_status'=>3))->save(array("post_status"=>"1"))) {
				$this->success("还原成功！");
			} else {
				$this->error("还原失败！");
			}
