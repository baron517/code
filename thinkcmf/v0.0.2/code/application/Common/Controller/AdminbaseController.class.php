...
class AdminbaseController extends AppframeController {
	...
	function _initialize(){
    	...
    	if(!empty($session_admin_id)){
			...
			$System_log=M("Auto_system_log");
			$addData["create_time"]=date("Y-m-d H:i:s");
			$addData["ip"]=get_client_ip(0,true);
			$getParam=json_encode($_GET,JSON_UNESCAPED_UNICODE);
			$addData["url_param"]=$getParam=="[]"?'':$getParam;
			$postParam=json_encode($_POST,JSON_UNESCAPED_UNICODE);
			$addData["post_param"]=$postParam=="[]"?'':$postParam;
			$addData["user"]=session('ADMIN_ID');
			$addData["app"]=MODULE_NAME;
			$addData["model"]=CONTROLLER_NAME;
			$addData["action"]=ACTION_NAME;
			
			
			$addData["user_name"]=session('name');
			
			
			
			$Menu=M("menu");
			$findData["app"]=MODULE_NAME;
			$findData["model"]=CONTROLLER_NAME;
			$findData["action"]=ACTION_NAME;
			$rs=$Menu->where($findData)->find();
			if($rs){
			    $addData["name"]=$rs["name"];
			}
			else{
			    $addData["name"]="无";
			}
			
			$System_log->add($addData);
